...
(() => {
  ...
  const DB_VERSION = 3;   // must track bg.js — a lower version here throws VersionError
  ...
  function open() {
    ...
    openPromise = new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = () => {
        ...
        // Mirrors bg.js: which account a chat came from, paired with its
        // revision. Whichever context opens the database first must create the
        // same shape, or the other one upgrades into a store it cannot query.
        if (!store.indexNames.contains("acctRev")) store.createIndex("acctRev", ["acct", "sourceUpdatedAt"]);
      };
